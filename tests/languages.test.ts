...
describe("language proficiency parsing", () => {
  it("parses CEFR levels and descriptive proficiency words", () => {
    const found = parseLanguageMentions("Languages: German (C1), fluent Spanish, native English speaker");
  ...
  it("flags a required language missing from the resume", () => {
    const result = analyzeResume({
      resumeText: "Summary: Software engineer.\nSkills: javascript\nExperience:\nEngineer at Foo (2020 - 2022)\nEducation: BS CS",
      jobDescription: "Must have JavaScript. German (B2) required for this role.",
    });
    expect(result.missingLanguages).toEqual([{ name: "german", level: "b2", levelRank: 4 }]);
